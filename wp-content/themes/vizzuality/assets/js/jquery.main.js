...
function initGeolocationMap() {
	var elem = document.getElementById('map'),
			mapHolder = jQuery(elem).closest('.m-map'),
			mapLocationItems = mapHolder.find('.m-map-location-item'),
			highlightedClass = 'is-highlighted',
			defaultPos = mapLocationItems.filter('.' + highlightedClass);
	...
	var mapOptions = {
		center: L.latLng(defaultPos.data('latlng').lat, defaultPos.data('latlng').lng),
		zoom: 8,
	...
	function defaultLocation() {
		var userPos = L.latLng(defaultPos.data('latlng').lat, defaultPos.data('latlng').lng);
		setLocation(userPos);
	}
	...
	function setLocation(userPos) {
		generateRoute(userPos, L.latLng(defaultPos.data('latlng').lat, defaultPos.data('latlng').lng));
		...
		mapLocationItems.on('click', locationsClickHandler);

		function locationsClickHandler(e){
			e.preventDefault();

			var curItem = jQuery(this);
			mapLocationItems.removeClass(highlightedClass);
			curItem.addClass(highlightedClass);

			generateRoute(userPos, L.latLng(curItem.data('latlng').lat, curItem.data('latlng').lng));
		}
